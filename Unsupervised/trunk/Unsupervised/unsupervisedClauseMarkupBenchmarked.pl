...
#July 28, 2011 Hong Cu$wptni How to create KB from checked wordroles and glossaryfixed tables
...
my $os = $ARGV[5]; #onto_structures table name
$prefix =~ s#-#_#g;
...
if($os ne "null" and $os ne "") {addontostructureterms($os)};
addheuristicsnouns();
...
#add single/multi-word terms to wordpos table as 'p'

sub addontostructureterms{
...
while(defined ($file=readdir(IN))){
	...
	$original = $text;
  	...
  	#$text =~ s#_#-#g;   #_ to -, because of prematched phrases (using _) from ontologies
