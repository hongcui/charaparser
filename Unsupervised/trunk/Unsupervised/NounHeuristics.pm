...
$STOP ="a|about|above|across|after|along|also|although|amp|an|and|are|as|at|be|because|become|becomes|becoming|been|before|behind|being|beneath|between|beyond|but|by|ca|can|could|did|do|does|doing|done|during|even|for|from|had|has|have|hence|here|how|if|in|into|inside|inward|is|it|its|may|might|more|most|near|no|not|of|off|on|onto|or|out|outside|outward|over|should|so|than|that|the|then|there|these|this|those|though|throughout|to|toward|towards|under|up|upward|via|was|were|what|when|where|whereas|which|why|with|within|without|would";
$ADVS ="always|often|seldom|sometimes|almost|never|altogether|still"; #advs not ending with -ly 
...
sub presentabsent{
	...
	if($text =~ /(\w+?)\s+(present|absent)/){
		my $n = $1;
		if($n !~ /\b($pachecked)\b/ && $n!~/\b($STOP)\b/ && $n !~/\b($ADVS|[a-z]+ly)\b/){
